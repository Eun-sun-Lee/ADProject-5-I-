...
class Puzzle_make:
    ...
    def make_puzzle(self):
        ...
        puzzle_board = [['_']*len(word1) for _ in range(len(word2))] # puzzle 생성 (모두 _으로 초기화)
        puzzle_board[common_y][common_x] = word1[common_x] # 겹치는 문자를 puzzle에 표시
        ...
        while i < (len(word1)-1)/2: # word1의 (len(word1)-1)/2 번 반복하며 puzzle_board에 문자를 표기
            ...
            puzzle_board[common_y][location_x[random_index]] = word1[location_x[random_index]] # puzzle board에 문자 표기
        ...
        while i < (len(word2)-1)/2: #  word2의 (len(word1)-1)/2 번 반복하며 puzzle_board에 문자를 표기
            ...
            puzzle_board[location_y[random_index]][common_x] = word2[location_y[random_index]]  # puzzle에 문자 표기
            location_y.pop(random_index) # 뽑은 index는 다음번 random 추출할때 제외하기 위해서 list에서 제거
            i += 1

        for i in range(len(puzzle_board)): # puzzle 출력부분
